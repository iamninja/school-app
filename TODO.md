...
- [ ] Production build fails: `parent-login-form.tsx` calls `useSearchParams()` without a `<Suspense>` boundary, which blocks prerendering under Next 16 Cache Components (`next build` errors on `/auth/parent-login`). Pre-existing, found 2026-08-14 while sanity-checking the ESLint fix with a real build. `student-login-form.tsx` has the same `useSearchParams()` pattern and is very likely affected too — needs checking.
